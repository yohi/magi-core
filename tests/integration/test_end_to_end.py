...
class TestPluginLoaderAsyncIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_load_all_async_respects_concurrency_limit(self):
        ...
        results = await loader.load_all_async(plugin_files, concurrency_limit=1, timeout=0.5)
...
class TestConsensusEngineIntegrationWithDI(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_execute_flow_with_injected_dependencies(self):
        ...
        class RecordingEmitter:
            ...
            async def emit(self, persona, chunk, phase, round_number=None, priority="normal"):
        ...
        budget_result = SimpleNamespace(context="trimmed ctx", summary_applied=False, logs=[])
        ...
        with patch.object(
            ...
            thinking_results = {
                PersonaType.MELCHIOR: ThinkingOutput(
                    persona_type=PersonaType.MELCHIOR, content="t1", timestamp=datetime.now()
                ),
                PersonaType.BALTHASAR: ThinkingOutput(
                    persona_type=PersonaType.BALTHASAR, content="t2", timestamp=datetime.now()
                ),
                PersonaType.CASPER: ThinkingOutput(
                    persona_type=PersonaType.CASPER, content="t3", timestamp=datetime.now()
                ),
            }
            debate_results = []
            ...
            agents = {
                persona: StubAgent(vote_output) for persona, vote_output in vote_outputs.items()
            }
