...
- **Findings**:
  - Anthropic: 軽量メッセージ送信は課金の可能性があるため、テンプレート一覧など非課金エンドポイントの有無を確認が必要（未確定）。
  - OpenAI: `/v1/models` 参照は非課金で利用可（要 API キー）。  
  - Gemini: モデルリスト取得エンドポイントが利用できるが無料かは確認要。
- **Implications**: design.md に「ヘルスチェックは非課金エンドポイントを優先」「不明時はスキップし警告のみ」「課金の可能性があればオプトイン」と記載する。実装前に各プロバイダの無料可否を確認するタスクを設計へ反映。
